 ...
 * @param {存储路径} filePath
 */
const commonFileSave = async (file, filePath) => {
	const fileExt = path.extname(file.name);
	const fileMd5 = getFileMd5(file.path);

	const fileName = fileMd5 + fileExt;
	const fileFileOption = {
		filepath: filePath,
		filename: fileName
	};
	await putFile(file, fileFileOption);
	return fileName;
};

module.exports = { putFile, delFile, fileExist, getFileMd5, commonFileSave };
